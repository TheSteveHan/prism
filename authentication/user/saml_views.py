...
def index(request):
    ...
    if 'sso' in req['get_data']:
        ...
        if not errors:
            ...
            login(request, user, backend='user.backends.saml.SAMLBackend')
            if req['post_data'].get('RelayState') and OneLogin_Saml2_Utils.get_self_url(req) != req['post_data']['RelayState']:
